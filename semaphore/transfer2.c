...
void produce() {
    ...
    while (readToEnd == 0) {
        ...
        if (sem_wait(&mutex) < 0)
            perror("producer mutex sem_wait"), exit(1);
        ...
        if (sem_post(&mutex) < 0)
            perror("producer mutex sem_post"), exit(1);

        if (sem_post(&items) < 0)
            perror("producer items sem_post"), exit(1);
...
void *consumer(void *params) {
    ...
    while(1){
        if (sem_wait(&items) < 0)
            perror("consumer items sem_wait"), exit(1);

        if (sem_wait(&mutex) < 0)
            perror("consumer mutex sem_wait"), exit(1);
        ...
        if(sem_post(&mutex) < 0)
            perror("consumer mutex sem_post"), exit(1);
    }
    return (void *)NULL;
}
...
int main(int argc, char* argv[]) {
    ...
    if (pthread_create(&consumer_thread, NULL, consumer, NULL) != 0)
        perror("pthread_create consumer"), exit(1);
    ...
    if (pthread_join(consumer_thread, NULL) != 0) {
        ...
        perror("pthread_join consumer"), exit(1);
    } else {
        ...
        if (sem_destroy(&mutex) < 0)
            perror("sem_destroy mutex"), exit(1);
        if (sem_destroy(&items) < 0)
            perror("sem_destroy items"), exit(1);
    }
